...
def make_ohlc(ticker,df):
    ...
    ohlc_fig.add_trace(go.Scatter(x=df['date'],y=df['200_sma'],name='200 SMA',line=dict(color='dodgerblue')),secondary_y=True)
    ...
    ohlc_fig.add_trace(go.Bar(x=df['date'], y=df['volume'],name='Volume',marker=dict(color='violet',opacity=0.10)),
                    secondary_y=False)
